...
import { hexStringToUint8Array } from "./bitcoin.utils.js";
import { wait } from "src/utils/index.js";
import { MempoolProvider } from './mempool.js';
...
// TODO_PERFORMANCE: Make 1 service for the Bitcoin wallet to reduce mempool queries on initiation.
export class BitcoinWallet {
    private mempoolProvider: MempoolProvider;
    ...
    private BITCOIN_DUST_LIMIT = 1000n; // sats.
    private hdkey: HDKey;
    ...
    constructor(mainnet: boolean, findCoins: boolean = true) {
        ...
        const rootKey = HDKey.fromJSON({ xpriv: config.BIP32_XPRIV });
    ...
    async updateLatestFee() {
        ...
        this.satsPerVirtualBytes = BigInt(fees.fatestfee);
    }
    ...
    async fetchCoins(from: number = 0) {
        ...
        for (let i = 0; i < this.coins.length; ++i) {
            if (this.coins[i].spentAt < now() - this.CLEAR_SPENT_COIN_FLAG_AFTER) this.coins.splice(i, 1);
        }
    }
