...
export class BitcoinWallet {
  ...
  private BIP32_XPRIV =
    "xprvA8YrhEjRG1XDnVEdwh4sfCtddnD9cNhTd46rSWSGk3SNm6Bhe79N3GsgvKoLe3M2JAvWve8FhpN1cLvA1oSxVhBVK6cSKaEo9s6DSigg5XR";
  ...
  private async initialize() {
    console.log(
      `Initializing Bitcoin wallet with pubKey: ${this.hdkey.publicExtendedKey}`,
    );
    ...
    if (this.findCoins) {
      ...
      console.log(
        `Finished Initializing Bitcoin wallet with pubKey: ${this.hdkey.publicExtendedKey}, containing: ${BitcoinWallet.utxoSum(this.coins) / 10 ** 8} Bitcoins`,
      );
      ...
      console.log(
        `Finished Initializing Bitcoin wallet with pubKey: ${this.hdkey.publicExtendedKey}, coin discovery disabled`,
      );
    }
  ...
  async updateLatestFee() {
    const fees = await this.mempoolProvider.retry(() =>
      this.mempoolProvider.bitcoin.fees.getFeesRecommended(),
    );
  ...
  async discoverAddresses() {
    ...
    this.addressDiscoveryIndex =
      this.addressLastInput.get(1) === undefined
        ? 1
        : this.goodToBeUsedAddressIndex;
    ...
    while (addressLastUsedAt != 0) {
      const { address: tryNextAddress } = this.getAddressAtIndex(
        this.addressDiscoveryIndex,
      );
      await mempoolWait;
      addressLastUsedAt =
        await this.mempoolProvider.addressLastUsedAt(tryNextAddress);
      mempoolWait = wait(this.MEMPOOL_WAIT_TIME);
      if (addressLastUsedAt > 0) {
        this.addressDiscoveryIndex += 1;
        this.addressLastInput.set(
          this.addressDiscoveryIndex,
          addressLastUsedAt,
        );
        continue;
  ...
  async recycleAddressIndex() {
    ...
    if (lowestKey != -1) {
      ...
      if (largestKey > this.goodToBeUsedAddressIndex)
        this.goodToBeUsedAddressIndex = largestKey + 1;
    }
  }
  ...
  async getNextSafeBitcoinAddress(amount: bigint): Promise<string | undefined> {
    ...
    for (let i = 0; i < this.MAX_TRIES_FOR_SAFE_ADDRESS; ++i) {
      ...
      // Don't spam mempool.
      await mempoolWait;
      const addressLastUsedAt =
        await this.mempoolProvider.addressLastUsedAt(tryNextAddress);
      console.log({
        tryNextAddress,
        addressLastUsedAt,
        clearenceTime: now() - this.UTXO_ORACLE_VALIDITY_PERIOD,
      });
      if (addressLastUsedAt > now() - this.UTXO_ORACLE_VALIDITY_PERIOD) {
        this.addressLastInput.set(
          this.goodToBeUsedAddressIndex + offset,
          addressLastUsedAt,
        );
      ...
      if (tried === undefined || tried === false) {
        ...
        if (addressMap === undefined)
          this.spendAddress.set(tryNextAddress, new Map<string, boolean>());
  ...
  getNextBitcoinAddress(offset: number): string {
    const { address: derivedWalletAddress } = this.getAddressAtIndex(
      this.goodToBeUsedAddressIndex + offset,
    );
  ...
  getAddressAtIndex(index: number): {
    ...
    const bitcoinWallet = ECPair.fromPrivateKey(privateKey, {
      network: this.getECPairNetwork(),
    });
  ...
  getInputs(
    ...
    candidateUxtos = candidateUxtos.filter((utxo) => {
      ...
      if (!utxo.status.confirmed) {
        ...
        const ourTransaction = this.ownTransactions.get(
          utxo.txid.toLowerCase(),
        );
    ...
    if (candidateUxtos.length === 0)
      return {
        inputs: selectedUxtos,
        value: BigInt(BitcoinWallet.utxoSum(selectedUxtos)),
      };
  ...
  async fetchCoins(from: number = 0) {
    ...
    for (
      let i = from;
      i < Math.max(this.addressDiscoveryIndex, this.goodToBeUsedAddressIndex);
      ++i
    ) {
      ...
      const knownUtxosForAddressIndex = this.coins.filter(
        (utxo) => utxo.pathIndex === i,
      );
      ...
      const spentUtxos = knownUtxosForAddressIndex.filter((knownUtxo) => {
        for (const stillUtxo of utxos) {
          if (
            stillUtxo.txid === knownUtxo.txid &&
            stillUtxo.vout === knownUtxo.vout
          )
            return false;
        }
        return true;
      ...
      for (let j = 0; j < this.coins.length; ) {
        ...
        for (const spentUtxo of spentUtxos) {
          if (
            examinedCoin.txid === spentUtxo.txid &&
            examinedCoin.vout === spentUtxo.vout
          ) {
      ...
      for (let j = 0; j < utxos.length; ++j) {
        ...
        const known = knownUtxosForAddressIndex
          .map((coin) => coin.txid === utxo.txid && coin.vout === utxo.vout)
          .indexOf(true);
        if (known != -1) {
          // Check if we need to clear spent flag.
          if (
            this.coins[j].spentAt <
            now() - this.CLEAR_SPENT_COIN_FLAG_AFTER
          ) {
  ...
  makeTransaction(
    ...
    if (spendInputs) {
      for (const input of inputs.inputs) {
        const indexOfCoin = this.coins
          .map((coin) => coin.txid === input.txid && coin.vout === input.vout)
          .indexOf(true);
    ...
    if (changeAmount > this.BITCOIN_DUST_LIMIT)
      psbt.addOutput({
        address: this.getAddressAtIndex(0).address,
        value: changeAmount,
      });
